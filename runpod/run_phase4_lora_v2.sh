#!/bin/bash
# run_phase4_lora_v2.sh — last-attempt LoRA Resonance Arianna SFT v2 driver.
#
# Plan: runpod/lora_plan_v2.md (v2.3, 5+ Codex passes, no Opus).
# Recipe: rank=32 alpha=64 lr=2e-4 ctx=512 steps=2500, 7 target modules per layer.
...
RUN_REPORT="$OUT/run_report_lora_v2_${TIMESTAMP}.txt"

mkdir -p "$OUT" "$OUT/sft_v2" "$OUT/slot_arianna"

log() { echo "[lora_v2 $(date +%T)] $*" | tee -a "$RUN_REPORT"; }

log "════════════════════════════════════════════════════════"
log "  LoRA SFT v2 — Resonance 200M + Arianna corpus"
...
log "─── step 1: toolchain + LoRA trainer build ───"
{
    ...
    log "  LoRA SFT v2 trainer build (USE_CUDA)..."
    cd "$NANOREPO/runpod"
    if ! cc -O3 -Wall -DUSE_BLAS -DUSE_CUDA lora_resonance_arianna_v2.c \
            -I/usr/local/include \
            -L/usr/local/lib -L/usr/local/cuda/lib64 \
            -lnotorch_gpu -laml \
            -lopenblas -lcudart -lcublas -lm -lpthread \
            -o lora_resonance_arianna_v2 2>&1 | tail -10; then
...
[ -x "$NANOREPO/runpod/lora_resonance_arianna_v2" ] || { log "FATAL: trainer binary"; exit 11; }
log "step 2 OK"

# ─── 3. Smoke-50 ─────────────────────────────────────────────────────────
SMOKE_PREFIX="$OUT/sft_v2/lora_v2_smoke50"
if [ "${SKIP_SMOKE:-0}" = "1" ]; then
    log "─── step 3: smoke-50 SKIPPED ───"
else
    log "─── step 3: smoke-50 LoRA SFT (50 steps, kill on bad signal) ───"
    "$NANOREPO/runpod/lora_resonance_arianna_v2" \
    ...
    rm -f "${SMOKE_PREFIX}_lora_v2_best.bin" "${SMOKE_PREFIX}_lora_v2_final.bin"
fi

# ─── 4. Full LoRA SFT ────────────────────────────────────────────────────
SFT_PREFIX="$OUT/sft_v2/resonance_arianna_lora_v2"
log "─── step 4: full LoRA SFT v2 ($SFT_STEPS steps) ───"
"$NANOREPO/runpod/lora_resonance_arianna_v2" \
    "$IN/resonance_200m_final.bin" \
    "$IN/arianna_dataset_final_clean.txt" \
    "$SFT_PREFIX" \
...
LORA_BEST="${SFT_PREFIX}_lora_v2_best.bin"
LORA_FINAL="${SFT_PREFIX}_lora_v2_final.bin"
if [ -f "$LORA_BEST" ]; then
    LORA_USE="$LORA_BEST"
...
MERGED_BIN="$OUT/sft_v2/resonance_arianna_lora_v2_merged.bin"
log "─── step 4.5: merge LoRA → $MERGED_BIN ───"
"$NANOREPO/runpod/lora_resonance_arianna_v2" --merge \
    "$IN/resonance_200m_final.bin" \
    "$LORA_USE" \
...
python3 "$NANOREPO/runpod/resonance_to_gguf.py" \
    "$MERGED_BIN" \
    "$OUT/slot_arianna/resonance_arianna_lora_v2_q8_0.gguf" \
    --quant q8_0 2>&1 | tail -5
rc=${PIPESTATUS[0]}
if [ "$rc" -ne 0 ]; then log "FATAL: Q8_0 quant exit $rc"; exit 14; fi
[ -s "$OUT/slot_arianna/resonance_arianna_lora_v2_q8_0.gguf" ] || { log "FATAL: Q8_0 gguf empty"; exit 14; }
log "  Q4_K..."
python3 "$NANOREPO/runpod/resonance_to_gguf.py" \
    "$MERGED_BIN" \
    "$OUT/slot_arianna/resonance_arianna_lora_v2_q4_k.gguf" \
    --quant q4_k 2>&1 | tail -5
rc=${PIPESTATUS[0]}
if [ "$rc" -ne 0 ]; then log "FATAL: Q4_K quant exit $rc"; exit 14; fi
[ -s "$OUT/slot_arianna/resonance_arianna_lora_v2_q4_k.gguf" ] || { log "FATAL: Q4_K gguf empty"; exit 14; }
log "step 5 OK"
...
EVAL_LOG="$OUT/sft_v2/lora_v2_eval.txt"
: > "$EVAL_LOG"
...
hf_upload_check "$LORA_USE"                                        "lora_v2/lora_arianna_v2_best.bin"
hf_upload_check "$OUT/slot_arianna/resonance_arianna_lora_v2_q8_0.gguf" "lora_v2/slot_arianna/resonance_arianna_lora_v2_q8_0.gguf"
hf_upload_check "$OUT/slot_arianna/resonance_arianna_lora_v2_q4_k.gguf" "lora_v2/slot_arianna/resonance_arianna_lora_v2_q4_k.gguf"
hf_upload_check "$RUN_REPORT"                                      "lora_v2/run_report_lora_v2.txt"
hf_upload_check "$EVAL_LOG"                                        "lora_v2/eval/multi_temp_eval.txt"
