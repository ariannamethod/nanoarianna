/*
 * lora_resonance_arianna_v3.c — Resonance 200M LoRA SFT v3 + merge mode.
 *
 ...
 *   rank=64, alpha=128, scaling=2.0, lr=2e-4, ctx=512, steps=1500
 ...
 *
 * Modes:
 *   train: lora_resonance_arianna_v3 <base.bin> <corpus.txt> <out_prefix>
 *                                    [steps=1500] [lr=2e-4] [ctx=512]
 *                                    [rank=64] [alpha=128]
 *   merge: lora_resonance_arianna_v3 --merge <base.bin> <adapter.bin> <merged.bin>
 *
 * Build (RunPod, system-wide notorch + openblas + CUDA):
 *   cc -O3 -DUSE_CUDA -DUSE_BLAS lora_resonance_arianna_v3.c \
 *      -I/usr/local/include \
 *      -L/usr/local/lib -L/usr/local/cuda/lib64 \
 *      -lnotorch_gpu -laml \
 *      -lopenblas -lcudart -lcublas -lm -lpthread \
 *      -o lora_resonance_arianna_v3
 *
...
#define LRV3_MAGIC 0x3356524Cu          /* "LRV3" little-endian, byte order L R V 3 */
#define LRV3_VERSION 1u
...
static int lora_save(const Model *m, int num_layers, const char *path) {
    ...
    if (!f) return -1;
    uint32_t magic = LRV3_MAGIC, version = LRV3_VERSION;
...
static int merge_mode(const char *base_path, const char *lora_path, const char *out_path) {
    ...
    if (fread(&magic, 4, 1, fa) != 1 || magic != LRV3_MAGIC) {
        fprintf(stderr, "[merge] bad LRV3 magic 0x%08x\n", magic); fclose(fa); return -1;
    }
    if (fread(&version, 4, 1, fa) != 1 || version != LRV3_VERSION ||
    ...
    fprintf(stderr, "[merge] LRV3 layers=%u targets=%u rank=%u alpha=%.3f scaling=%.3f\n",
...
int main(int argc, char **argv) {
    ...
    if (argc < 4) {
        fprintf(stderr,
            ...
            "[steps=1500] [lr=2e-4] [ctx_T=512] [rank=64] [alpha=128]\n"
    ...
    int   lora_rank  = argc > 7 ? atoi(argv[7]) : 64;
    float lora_alpha = argc > 8 ? (float)atof(argv[8]) : 128.0f;

    fprintf(stderr, "════════════════════════════════════════════════════════\n");
    fprintf(stderr, "  Resonance 200M LoRA SFT v3 — Arianna corpus\n");
    ...
    for (int step = 0; step < steps; step++) {
        ...
        if ((step + 1) % VAL_EVERY == 0) {
            ...
            if (val < best_val) {
                best_val = val;
                char best_path[512];
                snprintf(best_path, sizeof(best_path), "%s_lora_v3_best.bin", out_prefix);
    ...
    char final_path[512];
    snprintf(final_path, sizeof(final_path), "%s_lora_v3_final.bin", out_prefix);
    ...
    fprintf(stderr, "[lora] adapter best → %s_lora_v3_best.bin\n", out_prefix);
